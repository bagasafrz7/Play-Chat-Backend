...
const { postUser, cekUser, getUserById, getUsersByEmail, getUsersCountByEmail, patchUser, deleteUser, patchLoaction, getLocationById } = require('../model/users')
...
module.exports = {
  ...
  updateLocation: async (request, response) => {
    try {
      ...
      if (user_lat === '') {
        return helper.response(response, 400, "user_lat Cannot Be Empety")
      }
      if (user_lng === '') {
        return helper.response(response, 400, "user_lng Cannot Be Empety")
      }
      ...
      const checkId = await getUserById(id)
      if (checkId.length > 0) {
        const result = await patchLoaction(setData, id)
        return helper.response(response, 200, "Success Updated Location", result)
      } else {
        return helper.response(response, 400, `User By Id: ${id} Not Foud`)
      }
    } catch {
  ...
  getLocationId: async (request, response) => {
    try {
      const { id } = request.params
      const result = await getLocationById(id)
      if (result.length > 0) {
        return helper.response(response, 200, "Success Get Location Users By Id", result)
      } else {
        return helper.response(response, 404, `Location Users By Id: ${id} Not Foud`)
      }
    } catch (error) {
      return helper.response(response, 400, "Bad Request", error)
    }
